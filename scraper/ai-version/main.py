"""
AI-generated scraper for books.toscrape.com — round two, generated from
PROMPT_v2.md (the revised prompt naming the three gaps PROMPT_v1.md left open).
...
def fetch(url, cache_name):
    ...
    for attempt in range(2):
        ...
        # Fixed: only a timeout or a 5xx is worth a retry. A 404/403 is
        # final on the first try — the page won't exist just because we
        # asked twice, and retrying it is impolite for no benefit.
        if resp.status_code in (404, 403):
            print(f"giving up on {cache_name} (status {resp.status_code}, not retrying)")
            return None
...
def extract(url):
    ...
    # Fixed: tag["class"] is already a list of class names — pull the
    # rating word out of it directly instead of calling a string method
    # (.split()) on a list.
...
def save_books(records):
    ...
    # Fixed: overwrite the file with this run's full set every time,
    # instead of reading back whatever was already there and appending to
    # it. That's what makes a second run produce the same ~60 records
